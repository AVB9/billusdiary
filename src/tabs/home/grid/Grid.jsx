...
export default function Grid({ 
    ...
    const handleToggleWidget = (widgetType) => {
        const existingWidget = liveLayout.find(w => w.type === widgetType);
        
        if (existingWidget) {
            // TOGGLE OFF: Save to memory before deleting
            setLayoutMemory(prev => ({ ...prev, [widgetType]: existingWidget }));
            setLiveLayout(liveLayout.filter(w => w.type !== widgetType));
        } else {
            // TOGGLE ON: Check memory first
            if (layoutMemory[widgetType]) {
                setLiveLayout([...liveLayout, layoutMemory[widgetType]]);
            } else {
                // Spawn fresh if no memory exists
